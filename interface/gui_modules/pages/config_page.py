...
class ConfigPage(ctk.CTkScrollableFrame):
    ...
    def _construir_aba_astro(self, aba):
        ...
        for linha, texto, var, uni in campos:
            ctk.CTkLabel(aba, text=texto, font=fonte, text_color="#E5C07B").grid(row=linha, column=0, padx=20, pady=5, sticky="w")
            ctk.CTkEntry(aba, textvariable=var, font=fonte_var).grid(row=linha, column=1, padx=(10, 0), pady=5, sticky="sw")
            ctk.CTkLabel(aba, text=uni, font=fonte_uni, text_color="#8b949e").grid(row=linha, column=2, padx=(0, 50), pady=5, sticky="w")
    ...
    def _construir_aba_onda(self, aba):
        ...
        ctk.CTkCheckBox(aba, text="Use Constant Damping (Non-Resonant)", variable=self.app_state.cte).grid(row=6, column=0, columnspan=3, pady=15)
    ...
    def _construir_aba_gp(self, aba):
        ...
        for col, text in enumerate(headers):
            ...
            ctk.CTkLabel(aba, text=text, font=fonte, text_color="#E5C07B").grid(row=0, column=col, pady=(5, 5))

        for i, ref in enumerate(self.app_state.refs):
            row = i + 1
            ctk.CTkEntry(aba, textvariable=ref["x"], font=fonte_var, width=65, justify="center").grid(row=row, column=0, padx=5, pady=5)
            ctk.CTkEntry(aba, textvariable=ref["nome"], font=fonte_var, width=65, justify="center").grid(row=row, column=1, padx=5, pady=5)
            ctk.CTkEntry(aba, textvariable=ref["cor"], width=80, justify="center").grid(row=row, column=2, padx=5, pady=5)
            estilos = ["-", "--", "-.", ":"]
            ctk.CTkComboBox(aba, variable=ref["estilo"], font=fonte_var, values=estilos, width=70, state="readonly").grid(row=row, column=3, padx=5, pady=5)

        row_offset = len(self.app_state.refs) + 1

        ctk.CTkLabel(aba, text="Sigma Zone", font=fonte, text_color="#E5C07B").grid(row=row_offset, column=0, columnspan=4, pady=(10, 5))
        ...
        ctk.CTkEntry(frame_sigma, textvariable=self.app_state.sigma_ini, font=fonte_var, width=55).grid(row=0, column=1, padx=5)

        ctk.CTkLabel(frame_sigma, text="To:").grid(row=0, column=2, padx=(15, 5))
        ctk.CTkEntry(frame_sigma, textvariable=self.app_state.sigma_fim, font=fonte_var, width=55).grid(row=0, column=3, padx=5)

        ctk.CTkLabel(frame_sigma, text="Label:").grid(row=0, column=4, padx=(15, 5))
        ctk.CTkEntry(frame_sigma, textvariable=self.app_state.sigma_nome, font=fonte_var, width=50).grid(row=0, column=5, padx=5)

        ctk.CTkLabel(frame_sigma, text="Color:").grid(row=0, column=6, padx=(15, 5))
        ctk.CTkEntry(frame_sigma, textvariable=self.app_state.sigma_cor, font=fonte_var, width=75).grid(row=0, column=7, padx=5)

        row_offset += 2
        ctk.CTkLabel(aba, text="Axis Scales", font=fonte, text_color="#E5C07B").grid(row=row_offset, column=0, columnspan=4, pady=(10, 5))
        ...
        ctk.CTkComboBox(frame_axis, variable=self.app_state.axis["X Axis"], font=fonte_var, values=estilos_escala, width=100, state="readonly").grid(row=0, column=1, padx=5)

        ctk.CTkLabel(frame_axis, text="Y Axis Scale:").grid(row=0, column=2, padx=(30, 5))
        ctk.CTkComboBox(frame_axis, variable=self.app_state.axis["Y Axis"], font=fonte_var, values=estilos_escala, width=100, state="readonly").grid(row=0, column=3, padx=5)

        row_offset += 2
        self.btn_update_plot = ctk.CTkButton(aba, text="Update Plot", command=self.iniciar_replot, fg_color="#5C7174", state="disabled")
    ...
    def _construir_aba_more(self, aba):
        ...
        ctk.CTkLabel(aba, text="Advanced Execution Scripts", font=fonte_titulo, text_color="#E5C07B").grid(row=0, column=0, pady=(20, 15), sticky="w", padx=30)

        cb_multicurve = ctk.CTkCheckBox(
            aba, text="Run Multicurve Analysis (Resonant vs Constant Damping)", variable=self.app_state.multicurve,
            font=fonte_check, fg_color="#61AFEF", hover_color="#56B6C2", command=self._on_multicurve_toggle,
        )
        cb_multicurve.grid(row=1, column=0, padx=30, pady=10, sticky="w")
        ToolTip(cb_multicurve, "Executa a simulação duas vezes e sobrepõe os perfis\n de velocidade no mesmo gráfico.")

        cb_searchdv2 = ctk.CTkCheckBox(
            aba, text="Run DeltaV0² Search Script", variable=self.app_state.searchdv2,
            font=fonte_check, fg_color="#61AFEF", hover_color="#56B6C2", command=self._alternar_parametros_dv2,
        )
        cb_searchdv2.grid(row=2, column=0, padx=30, pady=(10, 0), sticky="w")
        ToolTip(cb_searchdv2, "Ativa o algoritmo de otimização para encontrar o melhor\n valor de amplitude inicial para as ondas de Alfvén. \nPode ser muito demorado.")

        self.frame_dv2 = ctk.CTkFrame(aba, fg_color="transparent")
        self.frame_dv2.grid(row=3, column=0, padx=(60, 30), pady=(5, 15), sticky="w")

        self.lbl_ldv2 = ctk.CTkLabel(self.frame_dv2, text="Lower limit:", font=fonte_check)
        self.lbl_ldv2.grid(row=0, column=0, padx=(0, 5))
        self.entry_ldv2 = ctk.CTkEntry(self.frame_dv2, textvariable=self.app_state.ldv2, font=fonte_var, width=60)
        self.entry_ldv2.grid(row=0, column=1, padx=(0, 20))

        self.lbl_hdv2 = ctk.CTkLabel(self.frame_dv2, text="Upper limit:", font=fonte_check)
        self.lbl_hdv2.grid(row=0, column=2, padx=(0, 5))
        self.entry_hdv2 = ctk.CTkEntry(self.frame_dv2, textvariable=self.app_state.hdv2, font=fonte_var, width=60)
        ...
        self.entry_stepdv2 = ctk.CTkEntry(self.frame_dv2, textvariable=self.app_state.stepdv2, font=fonte_var, width=60)
        self.entry_stepdv2.grid(row=0, column=5, padx=(0, 0))

        self.lbl_contagem_dv2 = ctk.CTkLabel(self.frame_dv2, text="Estimated runs: 0", font=("Roboto", 11, "normal"), text_color="#5c6269")
        self.lbl_contagem_dv2.grid(row=1, column=0, columnspan=6, pady=(8, 0), sticky="e")
    ...
    def _construir_aba_orbita(self, aba):
        ...
        ctk.CTkLabel(aba, text="Exoplanet Name:", font=fonte, text_color="#E5C07B").grid(row=0, column=0, padx=20, pady=5, sticky="w")
        ctk.CTkEntry(aba, textvariable=self.app_state.exoplanet_name, font=fonte_var).grid(row=0, column=1, columnspan=2, padx=(10, 0), pady=5, sticky="we")

        ctk.CTkLabel(aba, text="Orbital Distance ( Dorb ) :", font=fonte, text_color="#E5C07B").grid(row=1, column=0, padx=20, pady=5, sticky="w")
        entry_dorb = ctk.CTkEntry(aba, textvariable=self.app_state.Dorb, font=fonte_var, width=80)
        ...
        slider_dorb = ctk.CTkSlider(
            aba, from_=0.001, to=0.100, height=8, button_length=12,
            button_color="#61AFEF", button_hover_color="#56B6C2", progress_color="#1F618D",
            command=lambda v: self.app_state.Dorb.set(f"{v:.5f}"),
        )
        slider_dorb.grid(row=1, column=2, padx=(5, 10), pady=5, sticky="we")
        ctk.CTkLabel(aba, text=" [ AU ]", font=fonte_uni, text_color="#8b949e").grid(row=1, column=3, padx=(0, 20), pady=5, sticky="w")

        def update_dorb_slider(*args):
            try: slider_dorb.set(float(self.app_state.Dorb.get()))
            except ValueError: pass
        self.app_state.Dorb.trace_add("write", update_dorb_slider)
        update_dorb_slider()

        ctk.CTkLabel(aba, text="Eccentricity ( e ) :", font=fonte, text_color="#E5C07B").grid(row=2, column=0, padx=20, pady=5, sticky="w")
        ctk.CTkEntry(aba, textvariable=self.app_state.e, font=fonte_var).grid(row=2, column=1, columnspan=2, padx=(10, 0), pady=5, sticky="we")
        ctk.CTkLabel(aba, text="  [ adm ]", font=fonte_uni, text_color="#8b949e").grid(row=2, column=3, padx=(0, 20), pady=5, sticky="w")

        ctk.CTkLabel(aba, text="Albedo ( Ab ) :", font=fonte, text_color="#E5C07B").grid(row=3, column=0, padx=20, pady=5, sticky="w")
        ctk.CTkEntry(aba, textvariable=self.app_state.Ab, font=fonte_var).grid(row=3, column=1, columnspan=2, padx=(10, 0), pady=5, sticky="we")
        ctk.CTkLabel(aba, text="  [ adm ]", font=fonte_uni, text_color="#8b949e").grid(row=3, column=3, padx=(0, 20), pady=5, sticky="w")
    ...
    def _construir_aba_magnet(self, aba):
        ...
        ctk.CTkLabel(aba, text="Planet Radius ( Rplan ) :", font=fonte, text_color="#E5C07B").grid(row=0, column=0, padx=20, pady=5, sticky="w")
        ctk.CTkEntry(aba, textvariable=self.app_state.Rplan, font=fonte_var).grid(row=0, column=1, columnspan=2, padx=(10, 0), pady=5, sticky="we")
        ctk.CTkLabel(aba, text="  [ Rearth ]", font=fonte_uni, text_color="#8b949e").grid(row=0, column=3, padx=(0, 20), pady=5, sticky="w")

        ctk.CTkLabel(aba, text="Magnetic Moment ( Mmag ) :", font=fonte, text_color="#E5C07B").grid(row=1, column=0, padx=20, pady=5, sticky="w")
        entry_mmag = ctk.CTkEntry(aba, textvariable=self.app_state.Mmag, font=fonte_var, width=80)
        entry_mmag.grid(row=1, column=1, padx=(10, 5), pady=5, sticky="w")

        # MUDANÇA ESTÉTICA: height=8, button_length=12 para um slider mais fino e com botão redondo pequeno
        slider_mmag = ctk.CTkSlider(
            aba, from_=1e21, to=1e23, height=8, button_length=12,
            button_color="#61AFEF", button_hover_color="#56B6C2", progress_color="#1F618D",
            command=lambda v: self.app_state.Mmag.set(f"{v:.2e}"),
        )
        slider_mmag.grid(row=1, column=2, padx=(5, 10), pady=5, sticky="we")
        ctk.CTkLabel(aba, text="  [ Am² ]", font=fonte_uni, text_color="#8b949e").grid(row=1, column=3, padx=(0, 20), pady=5, sticky="w")

        def update_mmag_slider(*args):
            try: slider_mmag.set(float(self.app_state.Mmag.get()))
            except ValueError: pass
        self.app_state.Mmag.trace_add("write", update_mmag_slider)
        update_mmag_slider()

        ctk.CTkLabel(aba, text="Form Factor ( f0 ) :", font=fonte, text_color="#E5C07B").grid(row=2, column=0, padx=20, pady=5, sticky="w")
        ctk.CTkEntry(aba, textvariable=self.app_state.f0, font=fonte_var).grid(row=2, column=1, columnspan=2, padx=(10, 0), pady=5, sticky="we")
        ctk.CTkLabel(aba, text="  [ adm ]", font=fonte_uni, text_color="#8b949e").grid(row=2, column=3, padx=(0, 20), pady=5, sticky="w")
