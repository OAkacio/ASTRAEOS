...
# * ========================================================================================
# ? --- Identificação ---
nome_ = "TRAPPIST 1a"  # Nome da estrela analisada [string]
...
S_divergencia_ = 2.5  # Fator de expansão [adm]
F_ = 10  # Fator de expansão super-radial [adm]
phi0_ = 5e7  # Fluxo inicial de ondas Alfvén [erg/cm²/s]
L0_ = 1  # Comprimento de amortecimento [unidades de r0]
deltav0_ = 0.12851  # Amplitude de onda inicial [ve0²]
cte_ = False  # Flag de amortecimento constante [booleano]
parker_ = False  # Flag do modelo de Parker [booleano]
...
x_sim_ = 500  # Distância máxima de simulação [Rstar]
recuo_pulo_ = 484  # Número de passos recuados para o pulo [1e-5 Rstar]
tamanho_pulo_ = 0.1  # Tamanho do pulo [Rstar]
h_rk_ = 5e-4  # Passo de integração numérico [Rstar]

# ? --- Referências e Personalização ---
x_scale_ = "log"  # Escala do eixo x [string]
y_scale_ = "log"  # Escala do eixo y [string]
x_un_ = "r/r0"  # Unidade de exibição do eixo x [string]
y_un_ = "u/ve0"  # Unidade de exibição do eixo y [string]
x_ref_ = 37.0, 49.0, 63.0, 77.0  # Distâncias de referência [Rstar]
linestyle_ref_ = (
    ":",
    ":",
    ":",
    ":",
)  # Estilos das linhas de referência [tupla de strings]
color_ref_ = (
    "#922424",
    "#926224",
    "#927824",
    "#439224",
)  # Cores das linhas de referência em HEX [tupla de strings]
nome_ref_ = (
    rf"1d ; ${round(x_ref_[0],1)}$ $r_0$",
    rf"1e ; ${round(x_ref_[1],1)}$ $r_0$",
    rf"1f ; ${round(x_ref_[2],1)}$ $r_0$",
    rf"1g ; ${round(x_ref_[3],1)}$ $r_0$",
)  # Nomes dos pontos de referência gráficos [tupla de strings]
sigmas_ref_ = [[39.7, 84.7]]  # Limites gráficos da zona habitável [Rstar]
sigmas_color_ref_ = [
    "#8FFF81"
]  # Cor de preenchimento da zona habitável em HEX [lista de strings]
sigmas_nome_ref_ = [
    "ZH"
]  # Rótulo de texto da área de zona habitável [lista de strings]
...
exoplanet_name_ = (
    "TRAPPIST-1f"  # Nome do exoplaneta para análise de zona habitável [string]
)
...
# * ========================================================================================
# ? --- Parâmetros Magnéticos e Atmosféricos ---
